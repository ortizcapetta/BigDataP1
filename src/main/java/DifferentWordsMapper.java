 ...
 */
public class DifferentWordsMapper extends Mapper<LongWritable, Text, Text, IntWritable> {
    ...
    public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
	 	...
	 	s.close();*/
	 	Set<String> stopwords = new HashSet<String>(Arrays.asList("a", "about", "above", "above", "across", "after", "afterwards", "again", "against",
        ...
        for (String word: words) {
        	word = word.trim().replaceAll("[^A-Za-z0-9]", "").replaceAll("#","").replaceAll("[*0-9]", "");
        	if(!(stopwords.contains(word.toLowerCase()))&&!(word.toLowerCase().contains("http"))) {
